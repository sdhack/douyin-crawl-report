...
def extract(mp4, out_dir, fps, review_fps, scene_threshold, needs_review):
    os.makedirs(out_dir, exist_ok=True)
    meta_path = os.path.join(out_dir, "frames.json")
    config = {"fps": fps, "review_fps": review_fps, "scene_threshold": scene_threshold,
    ...
    try:
        ...
        step = max(1, int(round(rate / sample_fps)))
        idx = 0
        for frame in c.decode(video=0):
            ...
            if base_due or scene_due:
                ...
                n += 1
                last_saved = t
                records.append({"file": name, "timestamp": round(t, 3), "reason": "scene" if scene_due and not base_due else ("review" if needs_review else "base"),
...
def _worker(args):
    mp4, outdir, fps, review_fps, threshold, review_ids = args
    aid = os.path.splitext(os.path.basename(mp4))[0]
    n = extract(mp4, os.path.join(outdir, aid), fps, review_fps, threshold, aid in review_ids)
    return aid, n
...
def main():
    ...
    for jp in glob.glob(os.path.join(a.root, "transcript", a.account, "*.json")):
        try:
            ...
            pass
    tasks = [(mp4, fd, a.fps, a.review_fps, a.scene_threshold, review_ids)
