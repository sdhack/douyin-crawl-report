...
s3 02·达人画像（基本盘表 + 互动特征 + 时段 SVG 图）
s4 03·内容矩阵（主题表 + 爆款公式格 + TOP-N 真实封面表 + 关键帧切片）
s5 04·文案拆解（全 narrative：口号表/标题三段式/金句表/语料地图/带货结构/CTA）
s6 05·评论区实证（主题 hbar + 高赞 Top6 卡 + 摩擦原声分组）
s7 06·BGM×互动（组间表 + 情绪 + 轻/纯口播倍数）
s8 07·变现逻辑（叙事 steps + 推断型 caveat）
s9 08·起号方案（弱点清单 + 分层方案 + 合规红线清单）
footer 诚实口径附言（数据来源/缺源声明/合规边界，自动）
```

视觉系统锁定（rose/paper/serif 三色系 + SVG 图标系统 13 枚按关键词自动插入）；无主题轮换（那是博主总结渲染器的职责）。
