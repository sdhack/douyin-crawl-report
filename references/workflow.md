...
首次采集在父目录新建一个运行根，同时写运行根标记和父目录当前指针。此后同账号调用默认复用当前根，跨 Agent 同样有效；旧目录没有指针时只扫描父目录一级并选择最近有效根。评论可显式传 `--run-dir`，新一轮必须显式 `--new-run`。运行根创建使用账号级锁，避免并发 Agent 各建一个目录。
...
<root>/run.log|run-state.json              全流程追加日志、每分钟心跳与断点状态
