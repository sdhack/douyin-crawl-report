...
3. **内容分析**：优先用 `analyze.py` 一键执行“音频转写 → 自适应抽帧与 BGM 并行 → 逐帧指标/OCR”。基础 1 FPS，镜头突变补帧，`needs_visual_review` 视频提升到 5 FPS；每个视频生成 `frames.json` 时间轴（时间戳、镜头分数、亮度、清晰度、OCR 文本/状态），任一阶段失败立即退出。之后用 `bgm_cross.py` 做 BGM×互动交叉统计。
